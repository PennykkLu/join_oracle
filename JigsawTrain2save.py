...
def main():
    ...
    train_data = mydataset(data_path=datapath,
                           ...
                           kuai=3,
                           train=1,
                           daluan=0) #这个叫dataloader的其实是Dataset的子类
    ...
    #先用net把测试集的emb跑出来
    '''with torch.no_grad():
        for i, images in enumerate(train_loader):  # 这个for其实没有意义，一次会把整个batch都取完。
...
def test1(net,net2,output,val_loader,item_num,val_data): #这个item_num是有多少个整图，不要乘9
    print('Evaluating network.......')
    accuracy = []
    net.eval()
    net2.eval()
    with torch.no_grad():
        for i, images in enumerate(val_loader):  # 这个for其实没有意义，一次会把整个batch都取完。
            images = Variable(images)
            if args.gpu is not None:
                images = images.cuda()
